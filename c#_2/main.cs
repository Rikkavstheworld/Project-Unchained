...
class Program
{
    ...
    {
        Console.WriteLine("\nYour Guess History:");
        int tooLowCount = 0, tooHighCount = 0, correctCount = 0;

        foreach (var guess in guessHistory)
        {
            foreach (GuessResult result in Enum.GetValues(typeof(GuessResult)))
            {
                if (guess.Result == result)
                {
                    switch (result)
                    {
                        case GuessResult.TooLow: tooLowCount++; break;
                        case GuessResult.TooHigh: tooHighCount++; break;
                        case GuessResult.Correct: correctCount++; break;
                    }
                }
            }
        ...
        Console.WriteLine($"\nSummary of Guess Results:");
        Console.WriteLine($"Too Low: {tooLowCount}, Too High: {tooHighCount}, Correct: {correctCount}");
    }
}
